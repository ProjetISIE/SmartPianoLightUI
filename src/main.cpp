...
int main() {
    ...
    while (!WindowShouldClose()) {
            ...
            if (auto msg_opt = comm->popMessage()) {
                ...
                if (msg.getType() == "ack") {
                    if (msg.getField("status") == "ok") {
                        ...
                        current_state = GameState::CONNECTED;
                        challenge_text = "Config error: " + msg.getField("message");
                    }
                    ...
                    challenge_text = "Play chord: " + msg.getField("name") + " (" + msg.getField("notes") + ")";
                    current_state = GameState::CHALLENGE_ACTIVE;
                } else if (msg.getType() == "result") {
                    result_text = "Correct: " + msg.getField("correct") + " | Incorrect: " + msg.getField("incorrect");
                    current_state = GameState::SHOWING_RESULT;
                } else if (msg.getType() == "over") {
                    game_over_text = "Game Over! Total: " + msg.getField("total") + ", Perfect: " + msg.getField("perfect");
        ...
        if (IsKeyPressed(KEY_S)) { // Start Game
            if (current_state == GameState::CONNECTED || current_state == GameState::CONFIGURED || current_state == GameState::GAME_OVER) {
        ...
        if (IsKeyPressed(KEY_R)) { // Ready for next challenge
            if (current_state == GameState::CONFIGURED || current_state == GameState::SHOWING_RESULT) {
                Message ready_msg("ready");
                comm->send(ready_msg);
                challenge_text = "Waiting for challenge...";
                current_state = GameState::WAITING_FOR_CHALLENGE;
            }
        }
        if (IsKeyPressed(KEY_Q)) { // Quit game
            if(current_state != GameState::CONNECTED && current_state != GameState::DISCONNECTED) {
        ...
        switch (current_state) {
            case GameState::DISCONNECTED:
                DrawText("Connecting to engine...", screenWidth / 2 - 150, screenHeight / 2 - 10, 30, LIGHTGRAY);
                break;
            case GameState::CONNECTED:
                DrawText("Connected!", screenWidth / 2 - 80, screenHeight / 2 - 20, 30, LIME);
                DrawText("Press 'S' to Start a Note Game", screenWidth / 2 - 150, screenHeight / 2 + 20, 20, DARKGRAY);
                break;
            case GameState::WAITING_FOR_ACK:
                DrawText("Configuring game...", screenWidth / 2 - 120, screenHeight / 2 - 10, 20, DARKGRAY);
                break;
            case GameState::CONFIGURED:
                DrawText("Game configured!", screenWidth / 2 - 110, screenHeight / 2 - 20, 30, LIME);
                DrawText("Press 'R' for next challenge", screenWidth / 2 - 150, screenHeight / 2 + 20, 20, DARKGRAY);
                break;
            ...
            case GameState::SHOWING_RESULT:
                ...
                DrawText("Press 'R' for next challenge", 100, y_pos + 120, 20, DARKGRAY);
                break;
            case GameState::GAME_OVER:
                DrawText(game_over_text.c_str(), 100, y_pos, 40, BLUE);
                 DrawText("Press 'S' to Start a New Game", 100, y_pos + 60, 20, DARKGRAY);
                break;
        }

        // Draw general help text
        DrawText("S = Start, R = Ready, Q = Quit", 10, screenHeight - 30, 15, GRAY);
