...
/**
 * @brief Vérifie si une note est dans une liste (comparaison MIDI, gère les
 * enharmoniques)
...
/**
 * @brief Traduit une note en notation syllabique française avec altération et
 * octave
 * @param info Informations de la note
...
static constexpr int PIANO_WHITE_N =
    PIANO_OCT_COUNT * 7; ///< Touches blanches totales
...
void drawPiano(int x, int y, int totalW, int totalH,
    ...
    for (int oct = 0; oct < PIANO_OCT_COUNT; ++oct) {
        ...
        for (int wi = 0; wi < 7; ++wi) {
            float kx =
                static_cast<float>(x) + static_cast<float>(oct * 7 + wi) * ww;
            float ky = static_cast<float>(y);
            std::string noteStr(1, WHITE_LET[wi]);
            noteStr += std::to_string(octave);

            Color keyColor = KEY_WHI;
            if (!incorrect.empty() && noteInList(noteStr, incorrect))
                keyColor = KEY_BAD;
            else if (!correct.empty() && noteInList(noteStr, correct))
                keyColor = KEY_GOOD;
            else if (!challenge.empty() && noteInList(noteStr, challenge))
                keyColor = KEY_CHAL;
            ...
            if (wi == 0) {
                ...
                DrawText(lbl.c_str(),
                         static_cast<int>(kx + ww / 2) -
                             MeasureText(lbl.c_str(), sz) / 2,
    ...
    static constexpr char BLACK_LET[] = {'c', 'd', 'f', 'g',
                                         'a'}; // base pour le dièse
    for (int oct = 0; oct < PIANO_OCT_COUNT; ++oct) {
        int octave = PIANO_OCT_START + oct;
        for (int bi = 0; bi < 5; ++bi) {
            ...
            float ky = static_cast<float>(y);
            // noteInList via MIDI gère automatiquement les enharmoniques (ex:
            // f#3 == gb3)
...
bool drawToggleButton(Rectangle rec, const char* label, bool selected,
    ...
    DrawText(label, static_cast<int>(rec.x + rec.width / 2) - tw / 2,
             static_cast<int>(rec.y + rec.height / 2) - fontSize / 2, fontSize,
             selected ? WHITE : TEXT_PRI);
    return clicked;
}
...
bool drawActionButton(Rectangle rec, const char* label, Color color,
    ...
    Color bg = color;
    if (hovered) {
        bg.r = static_cast<unsigned char>(
            std::min(255, static_cast<int>(color.r) + 28));
        bg.g = static_cast<unsigned char>(
            std::min(255, static_cast<int>(color.g) + 28));
        bg.b = static_cast<unsigned char>(
            std::min(255, static_cast<int>(color.b) + 28));
    }
...
void drawFooter() {
    ...
    DrawText("S=Démarrer   R=Prêt   Q=Quitter   Esc=Fermer", 12, sh - 20, 13,
             TEXT_SEC);
}
...
void drawDisconnected() {
    ...
    const char* msg2 =
        "Verifiez que le moteur est lance sur /tmp/smartpiano.sock";
    DrawText(msg2, cx - MeasureText(msg2, 13) / 2, cy + 58, 13, TEXT_SEC);
}

/**
 * @brief Dessine l'écran de configuration avec sélection du mode, gamme et
 * tonalité
...
bool drawConfigScreen(GameConfig& cfg, const std::string& ackErr) {
    ...
    DrawRectangleRounded({static_cast<float>(cx), static_cast<float>(sy),
                          static_cast<float>(cw), 76.f},
                         0.06f, 6, BG_CARD);
    DrawText("MODE DE JEU", cx + 14, sy + 10, 13, TEXT_SEC);
    static const char* GAME_LABELS[] = {"Notes", "Accords",
                                        "Accords + Renversements"};
    int gBtnW = (cw - 40) / 3;
    for (int i = 0; i < 3; ++i) {
        Rectangle br = {static_cast<float>(cx + 14 + i * (gBtnW + 6)),
                        static_cast<float>(sy + 30), static_cast<float>(gBtnW),
                        32.f};
        if (drawToggleButton(br, GAME_LABELS[i], cfg.gameIdx == i))
            cfg.gameIdx = i;
    }
    sy += 84;
    ...
    static const char* SCALE_LABELS[] = {"Alea.", "Do",  "Re", "Mi",
                                         "Fa",    "Sol", "La", "Si"};
    int sBtnW = (cw - 40) / 8;
    for (int i = 0; i < 8; ++i) {
        Rectangle br = {static_cast<float>(cx + 14 + i * (sBtnW + 3)),
                        static_cast<float>(sy + 30), static_cast<float>(sBtnW),
                        32.f};
        if (drawToggleButton(br, SCALE_LABELS[i], cfg.scaleIdx == i, 15))
            cfg.scaleIdx = i;
    }
    sy += 84;
    ...
    for (int i = 0; i < 3; ++i) {
        Rectangle br = {static_cast<float>(cx + 14 + i * (mBtnW + 6)),
                        static_cast<float>(sy + 30), static_cast<float>(mBtnW),
                        32.f};
        if (drawToggleButton(br, MODE_LABELS[i], cfg.modeIdx == i))
            cfg.modeIdx = i;
    }
    sy += 84;
    ...
    Rectangle startBtn = {static_cast<float>(sw / 2 - btnW / 2),
                          static_cast<float>(sy + 4), static_cast<float>(btnW),
                          42.f};
...
void drawWaitingAck() {
    ...
    DrawCircleSector({static_cast<float>(cx), static_cast<float>(cy)}, 20.f, af,
                     af + 270.f, 36, COL_ACCENT);
...
bool drawConfigured(const GameConfig& cfg, bool& quitBtn) {
    ...
    static const char* GAME_NAMES[] = {"Notes", "Accords",
                                       "Accords + Renversements"};
    static const char* SCALE_NAMES[] = {"Alea.", "Do",  "Re", "Mi",
                                        "Fa",    "Sol", "La", "Si"};
    static const char* MODE_NAMES[] = {"Alea.", "Majeur", "Mineur"};
    std::string summary = std::string(GAME_NAMES[cfg.gameIdx]) +
                          "  |  Gamme: " + SCALE_NAMES[cfg.scaleIdx] +
                          "  |  Mode: " + MODE_NAMES[cfg.modeIdx];
    DrawText(summary.c_str(), cx - MeasureText(summary.c_str(), 17) / 2,
             cy - 28, 17, TEXT_SEC);

    Rectangle btnPlay = {static_cast<float>(cx - 160),
                         static_cast<float>(cy + 8), 148.f, 44.f};
    Rectangle btnQuit = {static_cast<float>(cx + 12),
                         static_cast<float>(cy + 8), 148.f, 44.f};
...
void drawChallenge(const ChallengeData& chall, bool& quitBtn) {
    ...
    DrawRectangleRounded({static_cast<float>(cx - bw / 2),
                          static_cast<float>(cy - 78), static_cast<float>(bw),
                          26.f},
                         0.4f, 6, badgeCol);
    ...
    if (!chall.id.empty()) {
        ...
        DrawText(idTxt.c_str(), cx - MeasureText(idTxt.c_str(), 14) / 2,
                 cy - 46, 14, TEXT_SEC);
    }

    // Label principal (nom de la note ou de l'accord)
    DrawText(chall.label.c_str(), cx - MeasureText(chall.label.c_str(), 38) / 2,
             cy - 18, 38, TEXT_PRI);

    // Détail des notes pour les accords
    if (chall.type == "chord" && !chall.notesRaw.empty()) {
        std::string notesTxt = "Notes: " + chall.notesRaw;
        DrawText(notesTxt.c_str(), cx - MeasureText(notesTxt.c_str(), 16) / 2,
                 cy + 32, 16, TEXT_SEC);
    }
    ...
    // Bouton Abandonner
    Rectangle qBtn = {static_cast<float>(cx - 82), static_cast<float>(cy + 86),
                      164.f, 38.f};
...
void drawResult(const ChallengeData& chall, const ResultData& result,
    ...
    DrawText(asked.c_str(), cx - MeasureText(asked.c_str(), 15) / 2, cy - 90,
             15, TEXT_SEC);
    ...
    if (!result.duration.empty()) {
        ...
        DrawText(durTxt.c_str(), cx - MeasureText(durTxt.c_str(), 15) / 2,
                 cy - 32, 15, TEXT_SEC);
    }
    ...
    if (!result.correct.empty()) {
        ...
        for (const auto& n : result.correct)
            okTxt += "  " + noteDisplay(parseNote(n));
        DrawText(okTxt.c_str(), cx - MeasureText(okTxt.c_str(), 18) / 2, cy - 4,
                 18, COL_OK);
    }
    ...
    if (!result.incorrect.empty()) {
        ...
        for (const auto& n : result.incorrect)
            errTxt += "  " + noteDisplay(parseNote(n));
        DrawText(errTxt.c_str(), cx - MeasureText(errTxt.c_str(), 18) / 2,
                 cy + 26, 18, COL_ERR);
    }

    // Boutons d'action
    Rectangle btnNext = {static_cast<float>(cx - 162),
                         static_cast<float>(cy + 62), 152.f, 40.f};
    Rectangle btnQuit = {static_cast<float>(cx + 10),
                         static_cast<float>(cy + 62), 152.f, 40.f};
...
void drawGameOver(const GameStats& stats, bool& playBtn, bool& menuBtn) {
    ...
    DrawRectangleRounded({static_cast<float>(cardX), static_cast<float>(cardY),
                          static_cast<float>(cardW), 134.f},
                         0.07f, 6, BG_CARD);

    auto drawStat = [&](const char* lbl, const std::string& val, int yOff,
                        Color col) {
        DrawText(lbl, cardX + 20, cardY + yOff, 17, TEXT_SEC);
        DrawText(val.c_str(), cardX + cardW - 20 - MeasureText(val.c_str(), 20),
                 cardY + yOff, 20, col);
    };
    drawStat("Durée", stats.duration.empty() ? "-" : stats.duration + " ms", 14,
             TEXT_PRI);
    drawStat("Total", stats.total.empty() ? "-" : stats.total, 42, TEXT_PRI);
    drawStat("Parfaits", stats.perfect.empty() ? "-" : stats.perfect, 70,
             COL_OK);
    drawStat("Partiels", stats.partial.empty() ? "-" : stats.partial, 98,
             COL_ERR);

    Rectangle btnPlay = {static_cast<float>(cx - 162),
                         static_cast<float>(cy + 100), 152.f, 42.f};
    Rectangle btnMenu = {static_cast<float>(cx + 10),
                         static_cast<float>(cy + 100), 152.f, 42.f};
...
void drawNotification(Notification& notif) {
    ...
    DrawRectangleRounded({static_cast<float>(nx), static_cast<float>(ny),
                          static_cast<float>(nw), 38.f},
                         0.2f, 6, bg);

    Color tc = {255, 255, 255, static_cast<unsigned char>(255.f * alpha)};
    DrawText(notif.message.c_str(),
             nx + nw / 2 - MeasureText(notif.message.c_str(), 15) / 2, ny + 12,
             15, tc);
}

// ─── Point d'entrée
int main(int argc, char* argv[]) {
    // Argument --timeout <ms> : fermeture automatique (utilisé pour la
    // couverture de code)
    int timeoutMs = 0;
    ...
    while (!WindowShouldClose()) {
        ...
        if (timeoutMs > 0 && (GetTime() - startTime) * 1000.0 > timeoutMs)
            break;
        ...
        switch (currentState) {
        case GameState::DISCONNECTED: drawDisconnected(); break;
        ...
        case GameState::WAITING_FOR_ACK: drawWaitingAck(); break;
        ...
        case GameState::WAITING_FOR_CHALLENGE: drawWaitingChallenge(); break;
        case GameState::CHALLENGE_ACTIVE: {
            bool quit = false;
        ...
        drawPiano(0, pianoY, sw, pianoH, chalNotes, correctNotes,
                  incorrectNotes);
