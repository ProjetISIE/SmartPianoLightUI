...
void Communication::send(const Message& msg) {
    ...
    if (::write(sock_fd, data.c_str(), data.length()) < 0) {
        ...
        Logger::log("[Comm] Sent: {}", msg.getType());
    }
}
...
void Communication::listen() {
    ...
    while (running) {
        ...
        if (bytes_read > 0) {
            ...
            while ((end_of_message_pos = current_message_data.find("\n\n")) != std::string::npos) {
                std::string msg_data = current_message_data.substr(0, end_of_message_pos);
    ...
    // Final disconnect if the loop terminates due to an error or server closing connection
    if(sock_fd != -1) {
        disconnect();
    }
}
